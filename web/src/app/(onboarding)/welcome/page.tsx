...
import {
  ONBOARDING_MODULES,
  dashboardLayoutForModules,
  hiddenSectionsForModules,
...
function WelcomeStep({ isInvited, householdName }: { isInvited: boolean; householdName: string }) {
  return (
    <div className="text-center space-y-5">
      ...
      <div className="space-y-2">
        <h2 className="text-2xl font-semibold">
          {isInvited ? "Welcome to Hearth" : "Welcome to Hearth"}
        </h2>
        <p className="text-sm text-muted-foreground leading-relaxed">
          {isInvited
            ? `You've been invited to join ${householdName ? `“${householdName}”` : "a household"}. Hearth helps households run smoothly — let's set it up the way you work.`
            : "Hearth helps households run smoothly — tasks, habits, meals, money, and everything else that keeps a home going. Let's set it up for you."}
        </p>
        <p className="text-sm text-muted-foreground">
          Four quick questions. Nothing here is permanent — you can change all of
...
/** Module id → card icon. The modules themselves (labels, what they reveal,
 *  which widgets they add) live in lib/onboarding/modules.ts, which stays free
 *  of React so the layout derivation can be reasoned about on its own. */
...
const PURPOSE_OPTIONS = ONBOARDING_MODULES;
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        {PURPOSE_OPTIONS.map((opt) => {
          const Icon = MODULE_ICONS[opt.id] ?? CheckSquare;
...
function DoneStep({
  ...
  return (
    <div className="text-center">
      ...
      {/* Arrow-wrapped, not passed directly: onSubmit takes an optional `skip`
          flag, and React would hand it the click event — a truthy first
          argument that would quietly finish the wizard as a skip. */}
...
export default function OnboardingPage() {
  ...
  async function handleNext() {
    ...
    if (currentStep.id === "purpose") {
      patch({
        hidden_sections: hiddenSectionsForModules(
          data.selectedPurposes,
          TOGGLEABLE_SECTIONS.map((s) => s.href),
        ),
      });
      setSectionsFromPurpose(data.selectedPurposes.length > 0);
    }
   ...
   * Finish the wizard.
   *
   * `skip` is the "Skip setup" escape hatch: it completes onboarding with
   * defaults — every section visible, the default dashboard, no module
   * preferences — rather than leaving the flag unset. Leaving it unset would
   * bounce the user straight back here on the next page load, which is not
   * skipping, it's a loop. Sample data is still offered on the skip path: the
   * whole point is not to strand someone in an empty app, and the dashboard
   * banner clears it in one click.
   */
  async function handleSubmit(skip = false) {
    setSubmitting(true);
    setError(null);
    try {
      const modules = skip ? [] : data.selectedPurposes;
      const preferences = {
        theme: data.themeConfig,
        sidebar: {
          hidden: skip ? [] : data.hidden_sections,
          order: [],
          folders: [],
        },
        // What the user said they care about drives the dashboard they land
        // on, not just the sidebar — an empty dashboard behind a filtered
        // sidebar is still the overwhelming blank app this step exists to
        // avoid. Selecting Finances and Habits lands you on a Budget widget
        // and a Habits widget.
        dashboard: dashboardLayoutForModules(modules),
      ...
      // Only the household owner can rename the household, and only when they
      // actually got as far as naming it.
      if (!isInvited && !skip && data.household_name.trim()) {
      ...
      if (modules.length > 0) {
        const wantsNotes = modules.includes("notes");
        const wantsTasks = modules.includes("tasks");
      ...
      // onboarding-002: offer to fill an empty household with explorable
      // sample content. The server decides — it declines with 200 if the
      // household already has content or already has sample data — so this is
      // safe to call unconditionally and safe to retry. A failure here must not
      // strand the user in the wizard: their profile is already saved, so we
      // swallow it and let them into an empty (but working) app.
      try {
        await apiClient.POST("/households/demo-data", {});
      } catch {
        // Non-fatal — sample data is a nicety, not part of setup.
      }
  ...
  const STEP_TITLES: Partial<Record<StepId, string>> = {
    // The welcome step carries its own heading and centred layout, so it gets
    // no title strip above it.
    household: isInvited ? "About you" : "Set up your home",
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="mb-8">
          {currentStep.id === "welcome"   && <WelcomeStep   isInvited={isInvited} householdName={data.household_name} />}
        ...
        {!isDone && (
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              {isInviteStep && isCloudTier ? (
                ...
                <Button onClick={handleNext} className="flex-1">
                  {currentStep.id === "welcome" ? "Get started" : "Continue"}
                </Button>
              )}
            </div>
            ...
            {/* Escape hatch out of the whole wizard. Completes onboarding with
                defaults rather than leaving the flag unset — an unset flag would
                land the user right back here, which is a loop, not a skip. */}
            {!isInviteStep && (
              <p className="text-center">
                <button
                  type="button"
                  ...
                  className="text-sm text-muted-foreground hover:text-foreground underline underline-offset-4 transition-colors disabled:opacity-40 disabled:pointer-events-none"
                >
                  {submitting ? "Skipping…" : "Skip setup"}
                </button>
              </p>
            )}
          </div>
