# Implementation Plans

Plans 001–010 generated by the `improve` skill on 2026-07-07 against commit `1977b97`.
Plans 011–013 added 2026-07-07 against commit `6ddaa9f` (post-merge of 001–010),
from the "Findings not yet planned" backlog — financial-integrity batch (audit #13/#19/#15).
Execute in the order below unless dependencies say otherwise. Each executor: read
the plan fully before starting, honor its STOP conditions, and update your row
when done.

These plans came out of a standard-depth audit of the Hearth codebase (correctness,
security, performance, tech-debt, tests, DX, direction). They are the top-leverage,
highest-confidence subset the maintainer selected — mostly S-effort with clean
verification stories. The full findings table (including lower-priority items not
yet planned) lives in the advisor session; a summary is in "Findings not yet planned"
below.

## Execution order & status

| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 001 | Remove committed secrets & data files, rotate credential | P1 | S | — | DONE & MERGED to `main` 2026-07-07 (commit `6cdce31` via merge `668f6bf`); leaked refresh token REVOKED by operator 2026-07-07 (`UPDATE refresh_tokens SET revoked_at = now()` on live DBs); token was also already past its 30-day TTL. History-purge deferred (low urgency now token is dead). |
| 002 | Establish verification baseline (tests + CI) | P1 | M | — | MERGED to `main` 2026-07-07 (commit `2b3102d` via merge `ef55b12`); 13 tests green |
| 003 | Fix cross-household budget-category write (goals) | P1 | S | 002 | MERGED to `main` 2026-07-07 (commit `8963eb7` via merge `a3835a0`) |
| 004 | Harden `environment` default to fail-safe | P1 | S | 002 | MERGED to `main` 2026-07-07 (commit `bff2e38` via merge `fa41316`); *`api/.env.example` doc line still deferred to operator (blocked by repo `.env.*` deny rule)* |
| 005 | Stop leaking internal exception details to clients | P1 | S | 002 | MERGED to `main` 2026-07-07 (commit `cf6c54f` via merge `e8423a7`) |
| 006 | Characterize recurring-todo completion (tests) | P1 | S | 002 | DONE & MERGED to `main` 2026-07-07 (commit `5e36143` via merge `a90e326`); `api/tests/test_todo_recurrence_completion.py`, 3 tests |
| 007 | Make recurring-todo completion atomic & idempotent | P1 | S | 006 (→002) | DONE (branch `advisor/007-atomic-recurring-todo-completion`; `update_todo` now takes `with_for_update()` row lock + gates spawn on `prev_status != "done"`; `test_todo_completion_idempotency.py` regression test; full suite 17 passed, 006 tests still green; changed files lint-clean. *Note: the `ruff check src tests → exit 0` criterion is unmet due to ~419 pre-existing `src/` lint errors — out of scope, tracked under the lint-debt follow-up; diff adds zero new errors, matching the CI gate which lints `api/tests` only.*) — MERGED to `main` 2026-07-07 (commit `e7ff7ac` via merge `a5424e1`) |
| 008 | Batch note backlink resolution (N+1) | P2 | S | 002 | DONE (branch `advisor/008-batch-note-backlink-resolution`; `_resolve_backlinks` now resolves all wikilinks in one `IN` query instead of one per link, `setdefault` for first-match-wins; `test_note_backlinks.py` 2 tests; full suite 19 passed; diff lint-clean, same pre-existing-`src`-debt caveat as 007) — MERGED to `main` 2026-07-07 (commit `70607ee` via merge `44be507`) |
| 009 | Batch auto-budget aggregation (N+1) | P2 | S | 002 | DONE (branch `advisor/009-batch-auto-budget-aggregation`; `auto_budget_fixed_categories` inner per-category loop → one `group_by(category_id)` query per period (periods×cats → periods round-trips); `test_auto_budget.py` 2 tests (average + transfer-exclusion); full suite 21 passed; ruff error count unchanged 33→33, none in edited region) — MERGED to `main` 2026-07-07 (commit `9426c7b` via merge `5148b3d`) |
| 010 | Unify income classification on `is_income` flag | P2 | S | 002 | DONE (branch `advisor/010-unify-income-classification`; `get_spending_trends` now classifies income by `BudgetCategoryGroup.is_income` (outer-join NULL → coalesce False → expense), matching `get_analytics`/`get_summary` instead of the hardcoded `name == "income"` match; misleading comment fixed; `test_income_classification.py` 2 tests (renamed group + uncategorized inflow); full suite 23 passed; ruff 33→32, zero new errors) — MERGED to `main` 2026-07-07 (commit `968cce9` via merge `6ddaa9f`) |
| 011 | Enforce import dedup at the DB (unique `(account_id, external_id)`) | P1 | M | 002 | DONE (branch `advisor/011-db-unique-import-dedup`; partial unique index `uq_budget_txn_account_external_id` on `BudgetTransaction.__table_args__` + migration `0042` with duplicate pre-check that fails loudly; `bulk_import_transactions` wraps each insert in `begin_nested()` and converts `IntegrityError` → skip; `test_import_dedup_constraint.py` 3 tests; full suite 26 passed; alembic single head `0042`; ruff: tests clean, `src` unchanged (service 32→32, models 11→11). *Note: the IntegrityError→skip conversion is verified by the DB-reject test + review; the true cross-session race isn't unit-testable single-threaded (cf. 007's lock).*) — MERGED to `main` 2026-07-07 (commit `421c0ca` via merge `507c5f6`); **migration `0042` applied to Railway Postgres 2026-07-07 (`alembic current` → `0042 (head)`)**; local is SQLite (schema via `create_all`, alembic no-op — fresh local DBs get the index from the model). |
| 012 | Guard `create_transaction` against double-submit (retry window) | P1 | S | 002 | DONE (branch `advisor/012-create-transaction-retry-guard`; `create_transaction` now checks for an identical `(account_id, dedup_hash)` row created within `CREATE_TXN_RETRY_WINDOW_SECONDS=120` and returns it instead of inserting a second money row; no `dedup_hash` unique constraint (legit same-day duplicates preserved); `test_create_transaction_retry_guard.py` 3 tests (retry collapses, distinct both insert, old duplicate not blocked); full suite 29 passed; ruff tests clean, `src` unchanged 32→32) — unmerged |
| 013 | Roll back session on `_maybe_check_thresholds` failure | P2 | S | 002 | DONE — **with deviation** (branch `advisor/013-threshold-check-rollback`). Plan prescribed `await db.rollback()` in the except block; **verified that regresses `create_transaction`** — a bare rollback expires the caller's just-created `txn`, so `model_validate(txn)` raises on the failure path. Instead wrapped the threshold check in a `begin_nested()` SAVEPOINT: a failure rolls back only its partial work, leaving the outer transaction clean and caller objects intact — achieving the plan's stated goal without the regression. `test_threshold_check_rollback.py` 3 tests (helper leaves session usable; happy path; **real guard: `create_transaction` survives a threshold failure**); full suite 32 passed; tests lint-clean, `src` 32→32. Plan's literal `db.rollback()` Done-criterion intentionally not met. — unmerged |

| 014 | SQLite schema evolution for the desktop tier (ADR) | P1 | M | — | PROPOSED 2026-07-20 — ADR, not yet executed. Decision: stamp-at-head + forward-only Alembic on SQLite (B2), with versioned export/import (D) as escape hatch. Awaiting sign-off before `feature_list.json` entries are added. |
| 016 | Bank sync: Teller shutdown → provider abstraction (Quiltt) | **P1** | L | — | PROPOSED 2026-07-22. **Teller withdrew its API in early July 2026 — bank sync is likely already dark in production.** Plan is a provider abstraction (`bank_connections` table + `link_*` columns replacing six `teller_*` columns), not a vendor swap, because self-hosted needs a cheap second provider (SimpleFIN ~$15/yr) that Cloud does not. Bears a migration on head 0053 — cannot run concurrently with another migration-bearing branch. Blocked on confirming Quiltt's MAU-vs-connected-account billing unit. |
| 021 | Defer conversational journal/coach from Cloud v1 | — | — | — | **DECIDED 2026-07-22.** Deferral, not cancellation — `journal-001/002`, `coach-002/003/007` stay in the repo and stay available self-hosted. Plain journal *writing* ships on Cloud; the AI conversation partner does not. Reason is safety, not cost: no crisis handling exists anywhere in `ai/`, `rant` mode instructs "stay with them, do NOT reality-test" (wrong for a spiral), and inferred emotional state persists into a profile injected downstream. Side effect: expected AI spend 26% → 15% of an $8 plan. Doc lists the conditions for shipping it later. |
| 020 | Implement prompt caching | **P1** | S/M | — | READY TO BUILD 2026-07-22. Implementation companion to 019, verified against Anthropic's caching docs. Two explicit breakpoints (last tool definition → caches the ~15.6k tool array shared across all users; system block → tools+system) plus top-level automatic caching for message history. Whole functional change is one helper + a `kwargs` edit in `provider.py`. Adds `cache_creation_input_tokens` / `cache_read_input_tokens` to `ai_usage` (migration on 0053, `batch_alter_table`) — without them you cannot tell caching from a misleading drop in `input_tokens`. Expect 2.2–2.9× on chat. Do before tool subsetting: pure upside, no product risk. |
| 019 | AI cost control: tool payload, caching, metering | **P1** | M | 020 | PROPOSED 2026-07-22, blocks any paid launch. **`TOOL_DEFINITIONS` is 66 schemas ≈ 15,582 tokens shipped on every API call** — $0.047/call before any conversation, multiplied by up to 6 calls per turn via the tool loop. A typical chat turn costs ~$0.15, not the $0.018 plan 018 assumed (~8× off). Prompt caching is **not implemented** (`cache_control` absent from `ai/`); adding it is ~30 lines in `provider.py` for 2–3×. Bigger lever is tool subsetting. Journaling turns out *cheaper* than tool-using chat (journal mode already restricts tools to one) — so it is **not** the add-on candidate it appeared to be. `AiUsage` already records per-call tokens by `turn_kind`; measure before pricing. |
| 018 | Tier structure brainstorm (pricing models, AI metering) | P2 | — | 019 | BRAINSTORM 2026-07-22, nothing decided. Three candidate models (one plan + add-ons / three tiers / two tiers + add-on), AI unit economics against current Claude rates, the ×1.25 rule for annual-vs-monthly, and the BYOK-upgrade grievance. Headline finding: an $8/mo plan is fully consumed by ~444 Sonnet messages, so "unlimited AI" is not a keepable promise. Input for a later planning session. |
| 017 | Paid tier limits: linked-account cap + business accounting tier | P2 | S | 016 (limit 1) | PROPOSED 2026-07-22. Companion to the `FREE_TIER_MAX_PROFILES` removal — captures the two limits that constant was *trying* to express. Limit 1 blocked on Quiltt's billing unit; limit 2 needs an explicit open-core pledge decision. Neither blocks the marketing site. |

Status values: TODO | IN PROGRESS | DONE | BLOCKED (with one-line reason) | REJECTED (with one-line rationale) | PROPOSED (ADRs awaiting sign-off)

## Recommended sequencing

1. **001** first — it's the most time-sensitive (a live refresh token is committed;
   rotation is an operator action flagged in the plan) and depends on nothing.
2. **002** next — it creates the test harness (`api/tests/conftest.py`, the
   `db_session` fixture) and the CI gate that **every other plan below relies on**.
   Nothing except 001 should run before it.
3. **003, 004, 005** — independent security fixes; any order once 002 is done.
4. **006 then 007** — 006 writes the characterization tests that 007's refactor is
   verified against. Do NOT run 007 before 006.
5. **008, 009, 010** — independent quality fixes; any order once 002 is done.
6. **011, 012, 013** *(current batch — financial integrity)* — independent of each
   other; any order. 011 first is recommended: it carries the migration (0042) and
   the highest-severity hole (duplicate rows from overlapping Teller syncs). 012 and
   013 are S-effort and can run in parallel worktrees with 011.

All of 011/012/013 edit `budget/service.py` in **non-overlapping functions**
(`bulk_import_transactions` / `create_transaction` / `_maybe_check_thresholds`),
but if you run them in parallel worktrees, rebase carefully — that file is large
and changes often. 011 is the only one touching `models.py` and `migrations/`.

## Dependency notes

- **002 is a hard prerequisite for 003–010.** They all add tests that import the
  `db_session` fixture and the pytest setup that 002 creates. Running any of them
  first will fail at import/collection.
- **007 depends on 006** because 006's three characterization tests (single spawn,
  end_date cutoff, field copy) are the safety net proving 007's atomicity change
  didn't alter correct behavior.
- 003/004/005 have a soft dependency on 002 only for their *tests*; the source fixes
  themselves are independent. If 002 is blocked, the source edits can still be made,
  but do not mark those plans DONE without their tests.

## Build wave plan — `feature_list.json` fan-out (added 2026-07-20)

Covers **`feature_list.json` entries**, not plans 001–014. Derived from a codebase audit on
2026-07-20 and from ADR-014 (`plans/014-sqlite-schema-evolution.md`). Read this before
assigning concurrent agents — several apparent fan-out candidates are already built, and two
are blocked by a migration dependency that is not obvious from the feature entries alone.

### Already built — verify, do NOT rebuild

An audit on 2026-07-20 found these implemented and wired, sitting at `passes: false` only
because they await manual verification. **Assigning an agent to build them would be rebuilding
finished work.**

- **`onboarding-001`** — full 6-step wizard exists at
  `web/src/app/(onboarding)/onboarding/page.tsx` (954 lines): household → goals → nav/section
  selection → theme → invite → done.
- **The entire AI coach chain** — `coach-001`/`001b`/`002`/`003`/`004`/`005`/`006`/`007`,
  `chat-001`, `journal-001`, `journal-002`. `api/src/life_dashboard/ai/` holds
  `coach_service.py`, `profile_service.py`, `journal_signal_service.py`,
  `chat_context_resolver.py`, `tools.py`; `ai_router` is mounted in `main.py:556` with
  endpoints for `/profile`, `/profile/bootstrap`, `/profile/updates`, `/profile/versions`,
  `/journal/start`, `/journal/save`, `/chat`, `/coach/digest`, plus scheduled profile refresh
  (`main.py:368`). Frontend: `web/src/components/journal/journal-session.tsx`,
  `web/src/components/ai/ai-chat.tsx`.

Verification is the highest-leverage work available (~12 features potentially flipped for zero
build effort) — but "registered in `main.py`" is not proof of "works." Per the endpoint
smoke-test rule, execute each endpoint against a running API before flipping any flag.

### Genuinely unbuilt

`infra-003`, `infra-004`, `infra-005`, `pwa-001` (no manifest, no service worker, no PWA deps
in `web/package.json`), `workouts-001` (still the old `Workout` + `ExerciseEntry` model with
sets inside a JSON `metrics` blob), `onboarding-002`, `onboarding-003`, `ux-001` (no shared
EmptyState component — but see below: `ux-001` is subsumed by `onboarding-003` and should not
be built separately).

### `ux-001` is a subset of `onboarding-003` — do not build both

`ux-001` ("every domain list page shows a friendly empty state with a short message and a
call-to-action") is fully contained in `onboarding-003`, whose steps already require a rich
empty state with a CTA on `/todos`, `/habits`, `/recipes`, `/notes`, `/goals`, and `/calendar`
— *plus* first-visit hints on top.

Build **`onboarding-003`**. `ux-001`'s verification steps should then pass without additional
work: verify and flip it rather than assigning it to an agent. Per the root `CLAUDE.md`, do not
remove or rename the `ux-001` entry.

### Two scoping decisions that control how much can run in parallel

Migration serialization is the throttle on this whole plan. Two entries are written with an
either/or that decides whether they join the serialized migration queue or run free:

1. **`onboarding-003` hint dismissals** — the entry specifies "a `dismissed_hints` JSONB column
   on member settings (**or localStorage for stateless hints**)." The column means a migration
   and a place in the queue; localStorage keeps it web-only and fully parallel.
2. **`onboarding-002` demo flag** — "all demo records are tagged with a `demo: true` metadata
   flag" across todos, habits, budget, recipes, goals, and notes. A real column per domain is a
   large migration; reusing existing JSON metadata or tags avoids one entirely.

Decide both **before** assigning agents. Choosing localStorage and existing-metadata converts
one long serial chain into three genuine parallel tracks.

### Track 1 — the migration queue (strictly serial)

`infra-003` → `infra-004` → `workouts-001`

These must not overlap: each adds a migration, and the root `CLAUDE.md` forbids concurrent
migration-adding tasks.

- `infra-004` is blocked by `infra-003` (migrations must run on SQLite before the folded
  backfill means anything there).
- `workouts-001` is blocked by `infra-003` because its migration seeds ~60 global exercises.
  Pre-003 that seed never runs on SQLite, so Tauri users get an empty exercise library and a
  non-functional workouts module.
- `workouts-002` / `workouts-003` remain blocked behind `workouts-001` regardless.

### Track 2 — parallel now

`pwa-001` — manifest, service worker, responsive audit. No API surface, no migration, no
overlap with anything else in flight.

### Track 3 — parallel now, *conditional*

`onboarding-003` (subsuming `ux-001`) — parallel-safe **only if** hint dismissals use
localStorage. If it takes the `dismissed_hints` column, it joins the back of Track 1 instead.

### After `infra-004` merges

`onboarding-002` — it seeds "a note in the Journal collection" and so touches
`domains/collections/service.py`, the same file `infra-004` edits. Also gated on its demo-flag
scoping decision above.

`infra-005` sits outside all of this — P2/v1.1, no sequencing pressure.

### Sequential multi-feature sessions

One agent may build several dependent features in succession. The merge protocol constrains
branches and merges, not who performs them — and a single serial agent makes the "two builds
finish together" race structurally impossible.

Required loop, per feature:

```
worktree feat/<id> → build → make check → git fetch && git rebase origin/main
→ update feature_list.json + claude-progress.txt → merge --ff-only to local main
→ branch the NEXT feature off the UPDATED main → repeat
```

Branch each subsequent feature off **main after the previous one merged**, never off the
previous feature's branch. That is what keeps a new migration's `down_revision` parented to the
real head instead of needing a hand-fix.

`make check` is the full gate (`lint` + `test` + `tsc --noEmit`).

**Driving this with `/goal`.** Claude Code's built-in `/goal` (goal-based loop —
`code.claude.com/docs/en/goal`) suits this work well: an evaluator model re-checks the stop
condition each time the agent tries to finish, and it performs best against deterministic
criteria. `feature_list.json` `steps` plus `make check` are exactly that.

Use **one `/goal` per feature, not one spanning several.** A single goal covering
`infra-003`–`005` will tend to build them on one branch, breaking one-feature-per-branch and
skipping the rebase between merges that keeps a new migration's `down_revision` parented to
the real head. Example:

```
/goal build infra-003 per feature_list.json. Done when every verification step passes,
make check is clean, and the branch is merged to local main. Do not commit the
test-scaffolding migrations. Stop after 8 tries.
```

Then start the next feature's goal off the updated main. `/goal` with no arguments reports
turns and token usage so far.

`infra-005` should **not** ride along with 003/004 — it is P2/v1.1, breadth-across-every-domain
work rather than surgical infra, and shares no useful context with them.

### Gotcha — `infra-003` verification scaffolding

`infra-003`'s steps call for adding test migrations (one with a unique constraint, one with a
data backfill) to prove migrations now apply on SQLite. These are **scaffolding**. They must
not be committed into `api/migrations/versions/` — if they land, every future install replays
them and the chain is polluted. Use scratch revisions reverted after verification, or a pytest
fixture.

### Concurrency rule reminder

`infra-003`, `infra-004`, and `workouts-001` each add migrations, so per the root `CLAUDE.md`
protocol they must not run concurrently — Track 1 already serializes them. `onboarding-003` and
`onboarding-002` join that queue **only if** the scoping decisions above land on a DB column
rather than localStorage / existing metadata.

## Findings not yet planned (from the audit, for a future pass)

Recorded so they aren't re-audited from scratch. Roughly in leverage order:

- ~~**Teller/bulk-import dedup has no DB unique constraint**~~ → **planned as 011**. (Audit #13.)
- **`list_habits` loads all household habits then slices in Python** (`habits/service.py:256`)
  — unbounded query. M. (Audit #14.)
- ~~**`_maybe_check_thresholds` swallows exceptions after commit without rollback**~~
  → **planned as 013**. (Audit #15.)
- **`auto_categorize_transactions` unbounded load + O(txns×cats×keywords)**
  (`budget/service.py:2205`), runs after every Teller sync. M. (Audit #16.)
- **`python-jose` on the JWT critical path is stale/CVE-exposed** (`auth/tokens.py:3`) —
  migrate to PyJWT. S–M. (Audit #17.)
- ~~**`create_transaction` computes a dedup hash but never checks it**~~
  → **planned as 012**. (Audit #19.)
- **`get_spending_trends` issues 2 queries per month** (`budget/service.py:2729`) — a
  sibling N+1 to plan 009. S–M. (Audit #9.)
- **Project progress ring is a recursive client-side fetch waterfall**
  (`web/src/lib/projects/progress.ts:44`) — add a backend endpoint. M. (Audit #18.)
- **budget/ god-module split** (`service.py` 3360 lines/69 fns) — L; do *after* the
  budget bug-fixes above land, or they'll need re-homing. (Audit #20.)
- **Pre-existing lint debt excluded from the new CI gate** (surfaced during plan 002 execution): `ruff check src` → 422 errors; `npm run lint` (`web/src`) → 135 problems (86 errors). The CI gate (plan 002) deliberately lints only `api/tests` (blocking) and treats web ESLint as advisory (`continue-on-error`), so the gate is green today. A follow-up cleanup pass should fix these and then promote both to blocking. S–M each.
- Lower: README contradicts ROADMAP + stale "life-dashboard" name (`README.md:99`);
  `.env.example` bakes in a real LAN IP; dead 104KB of root one-off scripts; upload
  trusts client `Content-Type` (`uploads/router.py:69`); unauth `/debug/cors`
  (`main.py:582`); `dedup_hash` drops legit same-day CSV txns (`budget/service.py:2351`);
  running-balance double-count (needs Teller-timing verification); scoping/soft-delete
  boilerplate duplicated across 18 domains.

**Direction options** (maintainer's call, not bugs): finish the idempotency roadmap
(`api/CLAUDE.md:63` — umbrella for #13/#19 and the notification-dedup gap); ~~unblock the
free SQLite/desktop tier (`ROADMAP.md:107`)~~ → **ADR'd as 014**; add the Teller webhook receiver
(`TELLER_SIGNING_SECRET` is configured but unused); iCal calendar export (`ROADMAP.md:154`).

## Findings considered and rejected

Verified by-design during the audit — do not re-flag:

- CORS uses an explicit origin allowlist + a safe `tauri://` regex (browsers never send
  `Origin: tauri://`), not a wildcard. (`main.py:452`)
- Dev impersonation is triple-gated: env check + `_ADMIN_ROLES` + same-household target
  verification. (`households/router.py`) — the only real issue was the fail-open
  `environment` default, addressed by plan 004.
- Refresh cookie sets `HttpOnly` + `SameSite=Lax` + `Secure` (outside dev). (`auth/router.py:58`)
- JWT decode pins a single algorithm (no alg-confusion); `jwt_secret_key` is required with
  no hardcoded fallback. (`auth/tokens.py`) — library staleness is tracked separately (#17).
- Teller mTLS: decoded key written to a `mkdtemp` (0700) dir, key file `chmod 0600`; base64
  failures logged without echoing secret material. (`teller_client.py:70`)
- First-run setup endpoint gated on `user_count == 0`. (`setup/router.py:74`)
- Auth datetime comparisons correctly use `_as_aware()`. (`auth/service.py`)
- Frontend dependencies are current (Next 16.2.4, React 19.2.4, TanStack Query 5.x) — no
  major-version lag worth acting on.
- `notes/service.py:148` / `templates/service.py:286` "unscoped reads" are read-backs off
  an already-household-scoped parent — low risk; folded into the boilerplate-duplication
  follow-up rather than treated as an IDOR.
