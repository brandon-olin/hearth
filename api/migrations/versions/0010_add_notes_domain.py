...
def upgrade() -> None:
    ...
    if not _table_exists(conn, "notes"):
        ...
        if not _index_exists(conn, "ix_notes_household_id"):
            op.create_index("ix_notes_household_id", "notes", ["household_id"])
        if not _index_exists(conn, "ix_notes_updated_at"):
            op.create_index("ix_notes_updated_at", "notes", ["updated_at"])

    # ── note_tags ─────────────────────────────────────────────────────────────
    if not _table_exists(conn, "note_tags"):
        op.create_table(
            "note_tags",
            sa.Column("id", UUID(as_uuid=True), primary_key=True),
    ...
    # ── note_backlinks ────────────────────────────────────────────────────────
    if not _table_exists(conn, "note_backlinks"):
        op.create_table(
            "note_backlinks",
            sa.Column("id", UUID(as_uuid=True), primary_key=True),
        ...
        if not _index_exists(conn, "ix_note_backlinks_target"):
